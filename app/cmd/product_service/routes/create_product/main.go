...
func (h *handler) handleRequest(ctx context.Context, r Request) (Response, error) {
	...
	err := json.Unmarshal(data, productDTO)
	if err != nil {
	...
	newProduct, err := h.productSvc.CreateProduct(ctx, *authCtx, *productDTO)
	if err != nil {
		return transport.Failure(transport.ErrorResponse{
	...
	h.logger.Debug("Created Product", "product", newProduct)

	return transport.Success(newProduct, http.StatusCreated)
}
